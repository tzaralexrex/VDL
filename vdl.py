...
# Где взять: откройте видео в браузере → DevTools → Network, ищите "web.gvs+" / "po" / "gvs"
# и скопируйте значение из ответа плеера. Полные инструкции — см. PO-Token Guide yt-dlp wiki (https://github.com/yt-dlp/yt-dlp/wiki/PO-Token-Guide).
# Как использовать:
#  - В среде Windows (командная строка): 
#      setx YTDLP_PO_TOKEN "web.gvs+ВАШ_ТОКЕН"
#  - В PowerShell (текущая сессия):
#      $Env:YTDLP_PO_TOKEN = "web.gvs+ВАШ_ТОКЕН"
#  - Или передать в yt-dlp напрямую:
#      yt-dlp --extractor-args "youtube:po_token=web.gvs+ВАШ_ТОКЕН" <URL>
# Альтернатива (небезопасный обход): formats=missing_pot — иногда работает, но даёт нестабильные/битые потоки.
